  ...
  return (
    <main>
      ...
      <section className="bg-blue-900 text-white py-24 px-4 text-center">
        ...
        <Link
          to="/contato"
          className="bg-white text-blue-900 font-semibold px-8 py-3 rounded-full hover:bg-blue-50 transition-colors"
        >
          Fale Conosco
        </Link>
      </section>
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
          {estatisticas.map((item) => (
            <div key={item.label}>
              ...
              <p className="text-blue-200 mt-1">{item.label}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-20 px-4 bg-gray-50">
        <div className="max-w-5xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-blue-900 mb-12">Nossos Diferenciais</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {diferenciais.map((item) => (
              <div key={item.id} className="bg-white rounded-2xl p-8 shadow-sm">
                <h3 className="text-xl font-bold text-blue-900 mb-3">{item.titulo}</h3>
      ...
      <section className="py-20 px-4">
        <div className="max-w-5xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-blue-900 mb-12">Nossas Unidades</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {unidades.map((u) => (
              <div key={u.id} className="border rounded-2xl p-6 text-left hover:shadow-md transition-shadow">
                <h3 className="font-bold text-blue-900 mb-1">{u.nome}</h3>
                <p className="text-gray-600 text-sm">{u.endereco}</p>
                <p className="text-gray-400 text-sm">{u.cidade}</p>
              </div>
            ))}
          </div>
          <Link to="/unidades" className="mt-8 inline-block text-blue-700 font-semibold hover:underline">
